...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    while ($Basedata = $rep->fetch(PDO::FETCH_ASSOC)) {
        if ($Basedata['Name'] == $tournament_name && ($Basedata['History'] == 0 || $Basedata['History'] == 1)) {
    ...
    if ($bool == false) {
        ...
        $sql = "INSERT INTO tournaments(Name, game_id, Match_system, participant, Register_time, creator_id) VALUES (:name, :game_id,:Choice_system,:Choice_participant,:Register_Time, :player_id)";
        $rep = $conn->prepare($sql);
